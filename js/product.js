$(function () {
  ...
  const renderPagi = (length, limit, currentPage, options = {}) => {
    ...
    $(".page-link").click(function (_currentPage) {
      ...
      if (options["category_id"] !== undefined) {
        tempUrl =
          `${urlProduct}` +
          `?category_id=${options["category_id"]}&_page=${_currentPage}&_limit=${_limit}`;
      } else {
  ...
  $(".category-id").change(function (el) {
    if ($(this).data("filter")) {
      ...
      $.get(
        `${urlProduct}?category_id=${category_id}&_page=${_currentPage}&_limit=${_limit}`,
        (data) => {
          data.forEach((product) => {
            renderProduct(product);
          });
  ...
  const renderProduct = (product) => {
    ...
    const template = `
        ...
        <td><img src="../images/products/${product.image}" alt="IMG"></td>
        <td style="max-width: 400px;">${capitalize(product.name)}</td>
        <td>${numberWithCommas(product.price)} đ</td>
        
        <td>
          
          <a class="btn btn-edit btn-outline-primary" ><i class="fas fa-edit"></i></a>
          <a class="btn btn-delete btn-outline-danger"><i class="fas fa-trash-alt"></i></a>
        </td>
      </tr>`;
    ...
    // delete Product
    const delPrd = document.querySelector(
      `[data-id='${product.id}'] .btn-delete`
    );
    ...
    // edit product
    const editPrd = document.querySelector(
      `[data-id='${product.id}'] .btn-edit`
    );
    if (editPrd) {
      editPrd.addEventListener("click", (el) => {
        ...
        $("#edit-product img").attr("src", `../images/products/${product.image}`);
  ...
  $(".js-insert-attach").on("click", function () {
    ...
    let lasting = $("#attach-view-" + insertNames + " li")
      .last()
      .prev()
    ...
    if (lasting != "") {
      ...
      _html +=
        ...
        insertNames +
        '[]" multiple="multiple" class="form-control showImage d-none" data-time="' +
        time +
        '" data-name="' +
        insertNames +
        '" >';
      ...
      if (lasting != "") {
        ...
        for (const key in fileSelected) {
          if (key == 0) {
            ...
            let _html =
              '<li class="img-box " id="' + insertNames + lastModified + '">';
            _html +=
              '<span class="icon-close" data-key="' +
              key +
              '" data-parent="' +
              insertNames +
              time +
      ...
      $(".icon-close")
        ...
        .on("click", function () {
          if ($(this).is("[data-key]") && $(this).is("[data-parent]")) {
            ...
            if ($("#" + parent).length) {
              let rootDel = $("#" + parent).children(
                "input[type=hidden]:first"
              );
              let rootFile = $("#" + parent).children(
